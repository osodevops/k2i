...
|   | - Retry Logic    |    | - Read LSNs      |    | - Atomic Commit  |      |
|   +------------------+    +------------------+    +------------------+      |
|            |                       |                       |               |
|            v                       v                       v               |
|   +---------------------------------------------------------------------+  |
...
|                                                                             |
|   +------------------+    +------------------+    +------------------+      |
|   | Kafka Brokers    |    | Schema Registry  |    | Iceberg Catalog  |      |
|   |                  |    |                  |    |                  |      |
|   | - Bootstrap      |    | - Protobuf       |    | - REST           |      |
|   | - SASL/SSL       |    | - Descriptors    |    | - Hive           |      |
|   | - Consumer       |    | - Subject cache  |    | - Glue/Nessie    |      |
|   |   Groups         |    |                  |    |                  |      |
|   +------------------+    +------------------+    +------------------+      |
|                                                                             |
|   +------------------+                                                       |
...
`GCS/Azure*` indicates declared storage support. The current writer implementation still needs object-store creation wiring for those backends.
...
| **MessageDecoder** | Raw and Confluent-framed Protobuf decoding |
| **SchemaEvolution** | Additive Protobuf evolution and readiness gating |
| **HotBuffer** | In-memory storage with indexes and eviction |
| **TableReadRegistry** | Read LSNs, hot/cold scan state, and RPC visibility |
...
| **ReadRpcServer** | Optional Unix socket read-state protocol |
...
|--------|--------|-------------|
| `k2i_messages_consumed_total` | `topic`, `partition` | Messages consumed |
